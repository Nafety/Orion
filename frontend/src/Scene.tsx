...
import { OrbitControls, Stars, Image, Text, MeshDistortMaterial, Billboard } from '@react-three/drei';
...
function ArtistNode({ node, position, viewType, color, isSelected, onSelect, isVisible }: any) {
  ...
  useFrame((state) => {
    ...
    if (materialRef.current) {
      ...
      if (isSelected) materialRef.current.emissiveIntensity = 15;
      else if (viewType === 'all' && node.is_recent) {
        materialRef.current.emissiveIntensity = (Math.sin(state.clock.elapsedTime * 2) + 1) * 1.5 + 3;
      } else materialRef.current.emissiveIntensity = 2;
    }
    if (ringRef.current) ringRef.current.opacity = THREE.MathUtils.lerp(ringRef.current.opacity, targetOpacity, 0.1);

    // Floating animation
    ref.current.position.y = Math.sin(state.clock.elapsedTime * 0.5 + node.popularity) * 0.15;
    ...
    <group position={position}>
      <group ref={ref}>
        <mesh
          ...
          onClick={(e) => { if (!isVisible) return; e.stopPropagation(); onSelect(isSelected ? null : node); }}
        >
        ...
        {node.top_track && (
          <mesh rotation={[Math.PI / 2, 0, 0]}>
            <torusGeometry args={[size + 0.4, 0.03, 8, 16]} />
        ...
        {isVisible && (
          <Billboard follow>
            <Image url={node.image} position={[0, size + 1.2, 0]} scale={[1.5, 1.5]} transparent opacity={isSelected ? 1 : 0.7} />
            <Text position={[0, -size - 1.0, 0]} fontSize={0.35} color="white" maxWidth={4} textAlign="center" fillOpacity={1}>
...
function SceneContent({ data, viewType, onNodeSelect, selectedNode, activeFilter }: any) {
  ...
  const nodes = useMemo(() => {
    const groups = Array.from(new Set(data.map((d: any) =>
      viewType === 'all' ? (d.genres?.[0] || "Autres") : d.artist_name
    )));
    ...
    return data.map((item: any) => {
      const key = viewType === 'all' ? (item.genres?.[0] || "Autres") : item.artist_name;
  ...
  return (
    <>
      <Stars radius={150} depth={50} count={2000} factor={2} fade={false} speed={0.3} />
      ...
      {nodes.map((node: any, i: number) => {
        ...
        if (activeFilter) {
          if (viewType === 'all') {
            ...
            isVisible = isFallbackFilter ? hasNoGenres : node.genres?.includes(activeFilter);
          } else {
      ...
      <EffectComposer>
        <Bloom luminanceThreshold={0.25} intensity={0.8} mipmapBlur />
      </EffectComposer>

      <OrbitControls makeDefault autoRotate autoRotateSpeed={0.15} enableDamping minDistance={20} maxDistance={220} />
    </>
  );
}
...
export default function Scene(props: any) {
  return (
    <div className="h-full w-full bg-[#010103]">
      <Canvas camera={{ position: [0, 20, 100], fov: 40 }} dpr={[1, 2]} frameloop="demand" onPointerMissed={() => props.onNodeSelect(null)}>
        <SceneContent {...props} />
      </Canvas>
    </div>
  );
